...
# metadata
metadata = {
    'protocolName': 'Station B - RNA extraction - Bikop',
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    STEPS = { #Dictionary with STEP activation, description, and times
            ...
            16:{'Execute': True, 'description': 'Transferir elución a los pitufos'},
            }
    ...
    def finish_run():
        ctx.comment('Finalizando Protocolo')
    ...
    def blow_to_dry(pipet, destCol, vol, blow_rate, iterations, x_offset_dest = 0,  pickup_height = 0, blow_height = 0,  wait_time = 0):   
       for i in range(iterations):
            # Pickup Air gap
            pipet.move_to(destCol.top(z = pickup_height))
            pipet.air_gap(vol) #air gap
            if wait_time != 0:
                ctx.delay(seconds=wait_time, msg='Esperando durante ' + str(wait_time) + ' segundos.')
            # Blow
            d = destCol.top(z = blow_height).move(Point(x = x_offset_dest))
            pipet.dispense(vol, d, rate = blow_rate)
            if wait_time != 0:
                ctx.delay(seconds=wait_time, msg='Esperando durante ' + str(wait_time) + ' segundos.')
